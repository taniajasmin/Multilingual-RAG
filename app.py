...
def preprocess_text(text: str, language: str = "bn") -> str:
    ...
    if language == "bn":
        text = re.sub(r"[^\u0980-\u09FF\s।,.\-অ-ঔ]", "", text)  # Keep more Bangla characters
    else:
        text = re.sub(r"[^\w\s.,-]", "", text)
    return text.strip()
...
def initialize_rag(pdf_path: str) -> RetrievalQA:
    ...
    rag_chain = RetrievalQA.from_chain_type(
        llm=llm,
        chain_type="stuff",
        retriever=vector_store.as_retriever(search_kwargs={"k": 10}),  # Adjusted k value
        return_source_documents=True
    )
    return rag_chain
...
# Add an endpoint to re-initialize (optional)
@app.get("/reinit")
